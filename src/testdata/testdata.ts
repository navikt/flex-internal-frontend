...
export async function mockApi(opts: BackendProxyOpts): Promise<void> {
    ...
    if (validert.api == 'GET /api/v1/sykmeldinger/[uuid]') {
        res.status(200)
    ...
    if (validert.api == 'POST /api/v1/flex/fta-vedtak') {
        res.status(200)
        res.json({ opprettet: 23, feilet: 0 })
        res.end()
        return
    }
    ...
    if (validert.api == 'GET /api/v3/soknader/[uuid]/kafkaformat') {
        ...
        res.json({
            versjon: 1,
            soknadKafkaformat: {
                id: sykepengesoknaderTestdata.sykepengesoknadListe[0].id,
                data: sykepengesoknaderTestdata.sykepengesoknadListe[0],
    ...
    if (validert.api == 'POST /api/v1/flex/sigrun') {
        res.status(200)
        res.json([{ skatt: 'Masse sigrun data' }])
        res.end()
        return
    }
    ...
    if (validert.api == 'POST /api/v1/flex/fta-vedtak-for-person/opprett') {
        res.status(200)
        res.json({})
        res.end()
        return
    }
    if (validert.api == 'GET /api/v1/flex/fta-vedtak/ubehandlede') {
        res.status(200)
        res.json([
            {
                id: '123',
    ...
    if (validert.api === 'POST /api/v1/flex/arbeidssokerperioder') {
        res.status(200).json({
